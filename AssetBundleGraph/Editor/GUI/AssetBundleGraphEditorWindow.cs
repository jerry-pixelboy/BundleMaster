...
namespace UnityEngine.AssetBundles.GraphTool
{
    public class AssetBundleGraphEditorWindow : EditorWindow
    {
        ...
        private float refreshGraphStartTime;
        ...
        public NodeGUI GetNodeGUI(string nodeId)
        {
            var findNodeGUI = nodes.Find(node => node.Id == nodeId);
            return findNodeGUI;
        }
		 ...
		 */
        public static AssetBundleGraphEditorWindow Window
        {
            get
            {
		...
		*/
        ...
        private void Init()
        {
            ...
            this.titleContent = new GUIContent("AssetBundle Graph");
		...
		*/
        ...
        private void DrawGUIToolBar()
        {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                {
                    refreshGraph = true;
                    refreshGraphStartTime = Time.realtimeSinceStartup;
                    ShowNotification(new GUIContent("Refreshing..."));
                }
                if (refreshGraph && Time.realtimeSinceStartup - refreshGraphStartTime >= 0.1f)
                {
                    refreshGraph = false;
                    Setup(true);
                    RemoveNotification();
