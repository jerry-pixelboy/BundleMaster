...
namespace UnityEngine.AssetBundles.GraphTool
{
    ...
    public class Filter : Node, Model.NodeDataImporter
    {
        ...
        {
            List<string> containZhCnAssetPathList = new List<string>();
            if (incoming != null)
            {
                foreach (var ag in incoming)
                {
                    foreach (var assets in ag.assetGroups.Values)
                    {
                        foreach (var a in assets)
                        {
                            ...
                            if (ContainZhCh(targetPath))
                                containZhCnAssetPathList.Add(targetPath);
                        }
                    }
                }
            }
            ...
            if (containZhCnAssetPathList.Count > 0)
            {
                string pathStr = string.Empty;
                for (int i = 0; i < containZhCnAssetPathList.Count; i++)
                {
                    var path = containZhCnAssetPathList[i];
                    if (i != containZhCnAssetPathList.Count - 1)
                        pathStr += (path + "\n");
                    else
                        pathStr += path;
                }
                if (window != null)
					Debug.LogWarning(string.Format("[AssetGraph Error] Asset path contain chinese character.\n{0}", pathStr));
                else
                    Debug.LogWarning(string.Format("[AssetGraph Error] Asset path contain chinese character.\n{0}", pathStr));
            }
            else
        ...
        // The asset file path whether contains chinese character.
        private bool ContainZhCh(string assetPath)
        {
            bool isContained = false;
            char[] assetPathCharArr = assetPath.ToCharArray();
            for (int i = 0; i < assetPathCharArr.Length; i++)
            {
                if (assetPathCharArr[i] >= 0x4e00 && assetPathCharArr[i] <= 0x9fbb)
                {
                    isContained = true;
                    break;
                }
